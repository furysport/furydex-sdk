...
import { AssetInfo, Uint128 } from "@furyanetwork/furydex-contracts-sdk";
import { TokenInfoResponse } from "@furyanetwork/furydex-contracts-sdk/build/FuryswapToken.types";
...
export const findToTokenOnFuryBridge = (fromCoingeckoId: CoinGeckoId, toNetwork: NetworkChainId) => {
  return cosmosTokens.find(
    (t) =>
      t.chainId === "furybridge-subnet-2" &&
...
export const getTokenOnFurya = (coingeckoId: CoinGeckoId, decimals?: number) => {
  return furyaTokens.find(
...
export const getSwapType = ({
  ...
  if (fromChainId === "furya-1" && toChainId === "furya-1") return "Swap";
...
export const toSumDisplay = (amounts: AmountDetails): number => {
  // get all native balances that are from furybridge (ibc/...)
  let amount = 0;
