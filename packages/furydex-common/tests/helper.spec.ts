...
import { AIRI_CONTRACT, AVERAGE_COSMOS_GAS_PRICE, BTC_CONTRACT, MILKYBSC_FURYA_DENOM, FURY } from "../src/constant";
import {
  calculateMinReceive,
...
import { AmountDetails, TokenItemType, cosmosTokens, flattenTokens, furyaTokens } from "../src/token";
import fs from "fs";
...
describe("should helper functions in helper run exactly", () => {
  const amounts: AmountDetails = {
    ...
    ufury: "1000000", // 1
    milky: "1000000", // 1
    [MILKYBSC_FURYA_DENOM]: "1000000000000000000" // 1
  };

  it.each<[string, boolean]>([
    ["0x", false],
    ["furya1g4h64yjt0fvzv5v2j8tyfnpe5kmnetejvfgs7g", false],
  ...
  it("should get sub amount of evm token correctly and to sum display, to total display correctly", () => {
    ...
    expect(subAmounts).toEqual({
      [MILKYBSC_FURYA_DENOM]: "1000000000000000000"
    });
  });

  it.each<[TokenItemType, string]>([
    [
      flattenTokens.find((item) => item.coinGeckoId === "airight" && item.chainId === "furya-1")!,
      flattenTokens.find((item) => item.coinGeckoId === "airight" && item.chainId === "furya-1")!.contractAddress!
    ],
    [
      flattenTokens.find((item) => item.coinGeckoId === "cosmos" && item.chainId === "furya-1")!,
      flattenTokens.find((item) => item.coinGeckoId === "cosmos" && item.chainId === "furya-1")!.denom
    ]
  ...
  it.each<[CoinGeckoId, TokenItemType, string]>([
    ["airight", cosmosTokens.find((token) => token.coinGeckoId === "airight" && token.chainId === "furya-1")!, ""],
    ["tether", cosmosTokens.find((token) => token.coinGeckoId === "tether" && token.chainId === "furya-1")!, ""],
    ["tron", cosmosTokens.find((token) => token.coinGeckoId === "tron" && token.chainId === "furya-1")!, ""]
  ])("test-getTokenOnFurya-given-%s-should-receive-%j", (coingeckoId, expectedToken, err) => {
    try {
      expect(getTokenOnFurya(coingeckoId)).toEqual(expectedToken);
    } catch (error) {
  ...
  it("test-isFactoryV1-true", () => {
    const data = isFactoryV1([
      { native_token: { denom: FURY } },
      { token: { contract_addr: "furya10ldgzued6zjp0mkqwsv2mux3ml50l97c74x8sg" } }
    ]);
  ...
  it("getEvmAddress-happy-path", async () => {
    expect(getEvmAddress("furye1ny7sdlyh7303deyqtzpmnznvyzat2jtyxs3y0v")).toEqual(
  ...
  it.each<[CoinGeckoId, NetworkChainId, CoinGeckoId, NetworkChainId | undefined]>([
    ...
    ["airight", "0x38", "airight", "furybridge-subnet-2"],
    ["usd-coin", "0x01", "usd-coin", "furybridge-subnet-2"],
    ["tron", "0x2b6653dc", "tron", "furybridge-subnet-2"]
  ])(
    "test-findToTokenOnFuryBridge-when-universalSwap-from-Furya-to%s",
    (fromCoingeckoId, toChainId, expectedToCoinGeckoId, expectedToChainId) => {
      const toTokenTransfer = findToTokenOnFuryBridge(fromCoingeckoId, toChainId);
  ...
  ])("test-findToTokenOnFuryBridge-expect-undefined", (fromCoingeckoId, toChainId) => {
    const toTokenTransfer = findToTokenOnFuryBridge(fromCoingeckoId, toChainId);
  ...
  it.each<[string, string | undefined, AssetInfo, any]>([
    [FURY, "10", { native_token: { denom: FURY } }, { denom: FURY, amount: "10" }],
    [FURY, undefined, { native_token: { denom: FURY } }, undefined],
  ...
  it("test-parseTxToMsgsAndEvents", async () => {
    ...
    // got data from tx hash 9B435E4014DEBA5AB80D4BB8F52D766A6C14BFCAC21F821CDB96F4ABB4E29B17 Furya.
    ...
    expect(data[0].message).toMatchObject({
      sender: "furya16hv74w3eu3ek0muqpgp4fekhrqgpzl3hd3qeqk",
      contract: "furya1nt58gcu4e63v7k55phnr3gaym9tvk3q4apqzqccjuwppgjuyjy6sxk8yzp",
      msg: {
        execute_order_book_pair: {
          asset_infos: [
            {
              token: {
                contract_addr: "furya1lplapmgqnelqn253stz6kmvm3ulgdaytn89a8mz9y85xq8wd684s6xl3lt"
              }
            },
            {
              token: {
                contract_addr: "furya12hzjxfh77wl572gdzct2fxv2arxcwh6gykc7qh"
              }
            }
          ],
          limit: 100
  ...
  it.each<[string, NetworkChainId, { isValid: boolean; network?: string; error?: string }]>([
    ...
    [
      "furya1hvr9d72r5um9lvt0rpkd4r75vrsqtw6yq564yt",
      "furya-1",
      {
        isValid: true,
        network: "furya-1"
  ...
  it.each([
    ["furya12zyu8w93h0q2lcnt50g3fn0w3yqnhy4fvawaqz", "furya-1", true],
    ["furya1", "furya-1", false],
    ["", "furya-1", false],
  ...
  it.each<[string, AssetInfo | null]>([
    ["", null],
    ["ufury333", null],
    ["ufury", { native_token: { denom: "ufury" } }],
