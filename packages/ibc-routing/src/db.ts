...
import fs from "fs";
import { generateError, toObject } from "@furyanetwork/furydex-common";
...
export const sqlCommands = {
  create: {
    ...
    FuryaState: `create table if not exists FuryaState 
    (
        txHash varchar,
        height uinteger,
        prevState varchar,
  ...
  query: {
    ...
    furyBridgeStateByNonce: `
      SELECT * from FuryBridgeState where eventNonce = ?
      `,
    furyBridgeStateBySequence: `
      SELECT * from FuryBridgeState where packetSequence = ?
      `,
    furyaStateByPacketSequence: `
      SELECT * from FuryaState where packetSequence = ?
      `,
...
export abstract class DuckDB {
  ...
  abstract queryFuryBridgeStateByNonce(eventNonce: number): Promise<any>;
  abstract queryFuryBridgeStateBySequence(packetSequence: number): Promise<any>;
  abstract queryFuryaStateBySequence(packetSequence: number): Promise<any>;
...
export class DuckDbNode extends DuckDB {
  ...
  async queryFuryBridgeStateByNonce(eventNonce: number) {
    const result = await this.conn.all(sqlCommands.query.furyBridgeStateByNonce, eventNonce);
    if (result.length > 0) return result[0];
    return [];
  }

  async queryFuryBridgeStateBySequence(packetSequence: number): Promise<any> {
    const result = await this.conn.all(sqlCommands.query.furyBridgeStateBySequence, packetSequence);
    if (result.length > 0) return result[0];
    return [];
  }

  async queryFuryaStateBySequence(packetSequence: number) {
    const result = await this.conn.all(sqlCommands.query.furyaStateByPacketSequence, packetSequence);
    if (result.length > 0) return result[0];
    return [];
  }
...
export class DuckDbWasm extends DuckDB {
  ...
  async queryFuryBridgeStateByNonce(eventNonce: number) {
    const stmt = await this.conn.prepare(sqlCommands.query.furyBridgeStateByNonce);
  ...
  async queryFuryBridgeStateBySequence(packetSequence: number): Promise<any> {
    const stmt = await this.conn.prepare(sqlCommands.query.furyBridgeStateBySequence);
  ...
  async queryFuryaStateBySequence(packetSequence: number) {
    const stmt = await this.conn.prepare(sqlCommands.query.furyaStateByPacketSequence);
    const result = (await stmt.query(packetSequence)).toArray();
    if (result.length > 0) return result[0];
    return [];
  }
